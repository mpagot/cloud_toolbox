...
ssh -i $MYSSHKEY $MY_USERNAME@$MY_PUBIP_ADDR 'ssh '"${MY_USERNAME}@${MYNAME}-vm-01"' sudo crm configure primitive rsc_ip_00 ocf:heartbeat:IPaddr2 meta target-role="Started" operations \\\$id="rsc_ip_RES-operations" op monitor interval="10s" timeout="20s" params ip="'"${MY_FIP}"'"'  || test_die "Fails in create IPaddr2 resource"
