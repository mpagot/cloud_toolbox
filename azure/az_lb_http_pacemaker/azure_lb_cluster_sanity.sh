...
for NUM in $(seq $MY_NUM); do
  ...
  ssh_proxy $this_vm 'sudo ls -lai /var/log/cloud-init.log'
  ssh_proxy $this_vm 'sudo cat /var/log/cloud-init.log | wc -l'
  ssh_proxy $this_vm 'sudo cat /var/log/cloud-init.log | grep -v DEBUG'
  ssh_proxy $this_vm 'sudo cat /var/log/cloud-init.log | grep -v DEBUG | grep -E "Command|Exit"'
