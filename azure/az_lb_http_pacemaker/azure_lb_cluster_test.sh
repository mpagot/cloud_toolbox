...
ssh_node1 'ip a show eth0' | grep ${MY_FIP} || test_die "VirtualIP ${MY_FIP} should be on ${MYNAME}-vm-01"
...
ssh_node1 'ip a show eth0'
ssh_node2 'ip a show eth0' | grep ${MY_FIP} || test_die "VirtualIP ${MY_FIP} should be on ${MYNAME}-vm-02"
...
test_step "Check cluster immediately after the move"
ssh_node1 'sudo crm configure show' | grep -c cli-prefer- | grep 1 || test_die "Cluster should now have one cli-prefer-"
ssh_node1 'sudo crm configure show' | grep -E "cli-prefer-.*${MYNAME}-vm-01" || test_die "Cluster should now have one cli-prefer- with ${MYNAME}-vm-01"
ssh_node1 'sudo crm status'
